 ...
 */
server.tool(
  "jira_cloud_get_sprints",
  {
    boardId: z.number().describe("Board ID (use jira_cloud_get_boards to find)"),
  ...
  async ({ boardId, state, startAt, maxResults }) => {
    ...
    try {
      ...
      if (startAt !== undefined) params.startAt = startAt;
      if (maxResults !== undefined) params.maxResults = maxResults;

      const result = await client.agileRequest<{
        values: Array<Record<string, unknown>>;
        total: number;
      }>(auth.cloudId, "GET", `/board/${boardId}/sprint`, params);
...
logger.info("Jira Cloud project tools registered");
